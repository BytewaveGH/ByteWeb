        ...
        >
          {data.map(({ value, desc, id }) => (
            <TabPanel key={id} value={value} className="flex w-full flex-wrap px-2 justify-center ">
              {desc.map((skills) => {
                return (
                  <Card key={skills.id} className="mt-4 w-11/12  md:w-64 sm:grid sm:grid-col-2 h-32 flex flex-wrap mx-5 justify-center items-center ">
